var TOOL = {
  ...
  _prizeDrawSituation: function (data) {
    ...
    $.each(data, function(key, value){
      ...
      if (!flag) {
        console.log()
          var this_dom = $('.prize-item[data-prizeid="' + key + '"]')
          this_dom.children(':first').addClass("hide");
          this_dom.children(':last').removeClass("hide");
          console.log(this_dom.children(':last').children().eq(0).html(value.alt));
          this_dom.children(':last').children().eq(0).html(value.alt)
          this_dom.children(':last').children().eq(1).find('img').attr("src",value.img)
          this_dom.attr('data-href',value.url)
          this_dom.removeClass('active').addClass('active');
      }
    });
  },
  ...
  _prizeDrawDataInit: function (response) {
    ...
    var remainNum = response.data.remain_prizes.num;  // 剩余抽奖次数
      self._prizeDrawSituation(response.data.remain_prizes.status)
    self._prizeDrawSuccess(bigImgData, remainNum);  // 抽奖成功弹窗内容更新
    self._prizeDrawTurnCard(response);  // 抽奖成功弹窗内容更新
